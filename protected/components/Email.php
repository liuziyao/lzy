...
final class Email{
	...
	public function send($email_to, $subject, $message, $from = ''){
		...
		fputs($fp, "Subject: ".$this->subject($subject)."\r\n");
		fputs($fp, $header."\r\n");
		fputs($fp, "\r\n\r\n");
		fputs($fp, "$message\r\n.\r\n");
		$lastmessage = fgets($fp, 512);
	...
	private function header($from=''){
		...
		$header .= 'Message-ID: <'.gmdate('YmdHs').'.'.substr(md5($from.microtime()), 0, 6).rand(100000, 999999).'@'.$_SERVER['HTTP_HOST'].">{$this->email_delimiter}";
		return $header;
	}
